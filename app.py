...
def carica_utenti():
    """Carica tutti gli utenti non-admin da Firebase."""
    if not db:
        return {}
    utenti = {}
...
def crea_utente(username, password):
    """Crea un nuovo utente non-admin su Firebase con password hashata."""
    if db:
...
def elimina_utente(username):
    """Elimina un utente non-admin da Firebase."""
    if db:
        db.collection(COLLEZIONE_UTENTI).document(username).delete()


def richiedi_login(f):
    """Decorator: richiede che l'utente sia loggato, altrimenti reindirizza a login."""
    @wraps(f)
...
def richiedi_admin(f):
    """Decorator: richiede che l'utente sia admin, altrimenti reindirizza a login."""
    @wraps(f)
    def wrapper(*args, **kwargs):
...
def login():
    """Pagina di login: verifica credenziali admin o utenti non-admin da Firebase."""
    errore = None
    if request.method == "POST":
        ...
        # Verifica utenti non-admin da Firebase
...
@richiedi_admin
def admin():
    """Panel admin: crea/elimina utenti non-admin, visibile solo all'admin."""
    messaggio = None
    try:
        if not db:
...
@richiedi_login
def home():
    """Home page: mostra le prime pagine dei giornali. Il link admin compare solo se è_admin=True."""
    utente_attuale = session.get("utente")
    è_admin = (utente_attuale == ADMIN_USER)
    return render_template("index.html", giornali=NOMI, utente=utente_attuale, è_admin=è_admin)


@app.route("/debug")
def debug():
    """Endpoint debug: verifica l'estrazione degli URL immagini da ogni fonte."""
    risultati = {}
...
@richiedi_login
def img(nome):
    """Serve l'immagine della prima pagina del giornale richiesto."""
